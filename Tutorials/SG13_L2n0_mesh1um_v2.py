...
if not preview_only:  # start simulation 
    ...
    # Save Zin data to a file
    zin_output_file = os.path.join(sim_path, model_basename + '_zin.txt')
    with open(zin_output_file, 'w') as file:
        for i in range(len(f)):
            file.write(f"{f[i]:.16e} {Zin[i].real:.16e} {Zin[i].imag:.16e}\n")
    print(f"Zin data saved to {zin_output_file}")
