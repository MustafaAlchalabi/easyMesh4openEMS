...
def adjust_mesh_parameters(automesher, unique_xedges, unique_yedges, z_coords, diagonal_edges, mesh_data):
            ...
            if distance_smaller_than_min_cellsize:
                ...
                if ds < automesher.min_cellsize:
                    ...
                    if ds < 1:
                        automesher.mesh_res = round(1 * (automesher.num_lines - 1))
                    else:
...
def mesh_small_gaps(automesher, unique_edges, mesh_data, direction):
    ...
    if direction == 'z':
        for i in range(len(unique_edges) - 1):
            if abs(np.diff([unique_edges[i][0], unique_edges[i + 1][0]])) <= automesher.mesh_res_z and abs(np.diff([unique_edges[i][0], unique_edges[i + 1][0]])) >= automesher.max_res_z:
                ...
                if use_num_lines:
                    new_max_res_z = np.diff(np.linspace(unique_edges[i][0], unique_edges[i + 1][0], automesher.num_lines))
                    new_max_res_z = np.max(new_max_res_z)
                    max_res_list_z.append(new_max_res_z)
                else:
                    new_max_res_z = automesher.max_res_z
                zlines = SmoothMeshLines([unique_edges[i][0], unique_edges[i + 1][0]], new_max_res_z)
                if len(zlines) <= 4:
                    mesh_data[2] = list(mesh_data[2]) 
    ...
    if max_res_list_z:
        new_min_cellsize_z = min(max_res_list_z) - 0.25*min(max_res_list_z)
        if new_min_cellsize_z < automesher.min_cellsize_z:
                ...
                if abs(np.diff([unique_edges[i][0], unique_edges[i + 1][0]])) <= automesher.mesh_res and abs(np.diff([unique_edges[i][0], unique_edges[i + 1][0]])) >= automesher.max_res and abs(np.diff([unique_edges[i][0], unique_edges[i + 1][0]])) >= 1.5:
                    ...
                    for edge in unique_edges:
                        if edge[0] == unique_edges[i+1][0]:
                            ...
                            other_edgs_with_same_coordinate.append(edge)           
                    for other_edge in other_edgs_with_same_coordinate:
                        y1, y2 = unique_edges[i][1], unique_edges[i][2]
                        y1_next, y2_next = other_edge[1], other_edge[2]
                        # print('unique_edges[i][3], unique_edges[i + 1][3]:', unique_edges[i][3], unique_edges[i + 1][3])    
...
def handle_circular_segments(automesher, x_edges, mesh_data):
